...
def micro_f(gold, ann, args):
    ...
    for gold_story, ann_story in zip(gold, ann):
    
        assert gold_story["index"] == ann_story["index"]
        assert gold_story["primary_person_id"] == ann_story["primary_person_id"]
        ...
        for field_name in field_names:
            ...
            ann_entities = set([normalize(e.strip()) for e in ann_story[field_name].split(",") if e.strip() != ""])
            ...
            if len(gold_entities) == 0:
                empty_gold += 1
                if len(ann_entities) == 0:
                    empty_correct += 1
                else:
                    false_positives += len(ann_entities)
                continue # done!
            ...
            common = gold_entities & ann_entities
            true_positives += len(common)
            
            # ..what is left
            gold_entities = gold_entities - common
            ann_entities = ann_entities - common
            
            # fuzzy match
            if args.fuzzy_threshold != 1.0:
                fuzzy_pairs, gold_entities, ann_entities = fuzzy_match(gold_entities, ann_entities, threshold=args.fuzzy_threshold)
...
def main(args):
    ...
    with open(args.annotation, "rt", encoding="utf-8") as f:
        annotation_data = json.load(f)

    print("Fuzzy threshold:", args.fuzzy_threshold)

    micro_f(gold_data, annotation_data, args)
...
if __name__=="__main__":
    ...
    parser.add_argument("--annotation", default=None, required=True, help="Annotation file")
